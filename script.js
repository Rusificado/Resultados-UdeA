// script.js (corregido: show/hide por style.display + manejo emptySummary + bloqueo botón)
...
const cortePrograms = [
  { code: 'ING-SIS', name: 'INGENIERÍA DE SISTEMAS', minScore: 86.130 },
  { code: 'MED', name: 'MEDICINA', minScore: 85.383 },
  { code: 'LIC-LENG', name: 'LIC. LENG. EXT. (INGLÉS/FRANCÉS)', minScore: 82.747 },
  { code: 'QUIM-FAR', name: 'QUÍMICA FARMACÉUTICA', minScore: 81.670 },
  ...
  { code: 'TRAD', name: 'TRAD. INGLÉS-FRANCÉS-ESP', minScore: 79.310 },
  { code: 'INST-QUIR', name: 'INSTRUMENTACIÓN QUIRÚRGICA', minScore: 78.100 },
  { code: 'COM-AUD', name: 'COMUNICACIÓN AUDIOVISUAL Y MULTIMEDIA', minScore: 76.057 },
  { code: 'ODO', name: 'ODONTOLOGÍA', minScore: 75.818 },
  { code: 'BIOING', name: 'BIOINGENIERÍA', minScore: 75.543 },
  ...
  { code: 'COM', name: 'COMUNICACIONES', minScore: 66.540 },
  ...
  { code: 'TECN-REGEN-FAR', name: 'TECN. REGENCIA EN FARMACIA', minScore: 41.543 },
  ...
  { code: 'ING-IND-VIRT', name: 'ING. INDUSTRIAL VIRTUAL (MEDELLÍN)', minScore: 18.877 },
  { code: 'EST', name: 'ESTADÍSTICA', minScore: 17.913 },
  { code: 'MAT', name: 'MATEMÁTICAS', minScore: 16.224 },
  { code: 'ING-MAT', name: 'INGENIERÍA DE MATERIALES', minScore: 16.147 },
  { code: 'ING-AM-VIRT', name: 'ING. AMBIENTAL VIRTUAL (MEDELLÍN)', minScore: 12.555 },
  { code: 'ADM-AMB-SAN', name: 'ADMÓN. AMBIENTAL SANITARIA', minScore: 8.283 },
  { code: 'ING-TEL-VIRT', name: 'ING. TELECOMUNICACIONES VIRTUAL (MEDELLÍN)', minScore: 7.653 },
  { code: 'QUIM', name: 'QUÍMICA (otras sedes/áreas)', minScore: 6.743 },
  { code: 'ING-SAN', name: 'INGENIERÍA SANITARIA', minScore: 6.225 },
  { code: 'LIC-FIS', name: 'LICENCIATURA EN FÍSICA', minScore: 5.693 },
  { code: 'LIC-CN', name: 'LIC. EN CIENCIAS NATURALES', minScore: 5.596 },
  { code: 'ING-TEL-REG', name: 'ING. TELECOMUNICACIONES (REGIÓN)', minScore: 4.419 },
  { code: 'ARCH', name: 'ARCHIVÍSTICA', minScore: 2.862 },
  { code: 'TEC-AGRO', name: 'TEC. GESTIÓN AGROPECUARIOS', minScore: 1.836 },
  { code: 'GEST-SIS-SAL', name: 'GESTIÓN SISTEMAS INFORMACIÓN SALUD', minScore: 0.898 },
  { code: 'LIC-ED-ESP', name: 'LIC. EN EDUCACIÓN ESPECIAL', minScore: 0.268 }
];
...
/* ---------- UI: mostrar previsualización y empty summary ---------- */
function showPreview(parsed) {
  ...
  // aseguramos que la sección de previsualización sea visible (manejo inline + clase)
  if (previewSection) {
    previewSection.classList.remove('hidden');
    previewSection.style.display = 'block';
  }

  // mostrar resumen de vacíos
  showEmptyAnswersSummary(parsed);
}

function showEmptyAnswersSummary(parsed) {
  if (!parsed || !Array.isArray(parsed.emptyAnswers)) return;
  const empties = parsed.emptyAnswers.filter(n => Number.isFinite(n) && n > 0);
  const emptyRL = empties.filter(n => n >= 1 && n <= 40).length;
  const emptyCL = empties.filter(n => n >= 41 && n <= 80).length;
  const totalEmpty = emptyRL + emptyCL;

  const summaryEl = document.getElementById('emptySummary');
  if (!summaryEl) return;

  summaryEl.innerHTML = `
    <h4 style="margin:8px 0 4px 0;">Respuestas vacías:</h4>
    <div style="white-space:pre-line; font-family:inherit; font-size:0.95em;">
      Razonamiento Lógico: ${emptyRL} / 40
      Comprensión Lectora: ${emptyCL} / 40
      Total vacías: ${totalEmpty} / 80
    </div>
  `;
  summaryEl.style.display = 'block';
}

/* ---------- Form submit / buscar archivo ---------- */
searchForm.addEventListener('submit', async (e) => {
  ...
  if (submitBtn) submitBtn.disabled = true; // bloquear mientras consulta
  ...
  try {
    ...
    if (!fetched.ok) {
      msg.textContent = `No se pudo leer el archivo: ${candidateFile} (status ${fetched.status}). Revisa la carpeta resultados/ y CORS si aplica.`;
      console.error('fetch error', fetched);
      if (submitBtn) submitBtn.disabled = false;
      return;
    }
...
clearBtn.addEventListener('click', () => {
  // esconder preview y limpiar campos
  if (previewSection) {
  ...
  const existingSummary = document.getElementById('emptySummary');
